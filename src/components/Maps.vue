...
<template>
  <div>
    ...
    <div id="container">
      <!-- Buttons that change the map view to that of the chosen city -->
...
export default {
  ...
  components: {},
  data() {
    return { map: null, tileLayer: null, layers: [] };
  },
  mounted() {
    this.initMap();
    this.initLayers();
  ...
  methods: {
    initMap() {
      // Creates the map and markers and sets an initial view
      ...
      L.marker([58.159912, 8.018206], { title: "Kristiansand" }).addTo(
        this.map
      );
    ...
    changeLocation(lat, lon) {
      // Changes the map view to that of the currently chosen city's coordinates
